...
function Header() {
  ...
  useEffect(() => {
    ...
    // Fetch search results from API with search query
    const fetchSearchResults = async () => {
      try {
        const response = await axios.post(`/api/v1/user/getAllFoodBySearch`, {
          search: searchQuery,
        });
            ...
            <div className="relative flex items-center space-x-4">
              <div className="relative w-64">
                ...
                <input
                  ...
                  type="text"
                  placeholder="Search"
                  className="w-full bg-gray-100 text-gray-800 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-white border-2 border-orange-400"
                />
                ...
                {/* Dropdown Results */}
                {showDiv && (
                  <div className="absolute right-0 top-full mt-1 w-full bg-white shadow-lg rounded-lg p-2 z-50 border border-gray-200 max-h-60 overflow-y-auto">
                    <div className="space-y-2">
                      {searchResult.map((item) => (
                        <NavLink to={`/food/${item._id}`} key={item._id}>
                          <div
                            className="flex items-center p-2 border rounded-lg hover:bg-gray-100 cursor-pointer my-2"
              ...
              <button className="text-white">
                <Link to="/cart">
                  <svg
                    ...
                    fill="currentColor"
                  >
                    <path d="M528.12 301.319l47.273-208A16 16 0 0 0 560 64H128l-4.744-22.382A24 24 0 0 0 99.636 32H24A24 24 0 0 0 0 56v16a24 24 0 0 0 24 24h56l70.4 324.319A64 64 0 1 0 256 416h192a64 64 0 1 0 63.6-55.681zM160 464a32 32 0 1 1 32-32 32 32 0 0 1-32 32zm320 0a32 32 0 1 1 32-32 32 32 0 0 1-32 32z" />
                  </svg>
                </Link>
              </button>
            </div>
